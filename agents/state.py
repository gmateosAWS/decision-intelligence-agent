...
------
  ...
  awaiting_user_confirmation (bool)   -- True when proactive gate fired and the
                                          client must confirm before execution
                                          continues (item 5.13)
  proposal (Dict[str, Any] | None)    -- JSON-serialisable representation of the
                                          StateProposal generated by the gate;
                                          None when not awaiting confirmation
...
class AgentState(TypedDict, total=False):
    ...
    # Proactive confirmation gate (item 5.13) — set when signals fire for
    # an expensive tool; the client receives the proposal and must confirm.
    # proposal is stored as Dict (not StateProposal) for checkpointer safety.
    ...
    bypass_gate: bool
    # has_prior_turns: set True by runner.py when agent_sessions.turn_count > 1.
    # register_turn() increments turn_count before run_query() is called, so
    # turn_count > 1 means prior turns exist. We cannot use state["history"]
