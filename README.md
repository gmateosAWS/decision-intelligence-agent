...
Interactive docs available at [http://localhost:8000/docs](http://localhost:8000/docs) (Swagger UI) and `/redoc`.

### Endpoints

| Method | Path | Description |
|--------|------|-------------|
| `POST` | `/v1/query` | Run agent query (creates session if none provided) |
| `GET` | `/v1/sessions` | List sessions (paginated) |
| `GET` | `/v1/sessions/{id}` | Session detail with turn count |
| `DELETE` | `/v1/sessions/{id}` | Delete session |
| `GET` | `/v1/runs` | List agent runs (filterable by `session_id`) |
...
curl -X POST http://localhost:8000/v1/query \
  ...
  -d '{"query": "What price maximises profit?"}'
```
...
All business endpoints use the `/v1/` prefix. Breaking changes will be introduced under `/v2/` — existing integrations are never silently broken.

### CORS

Configured via `CORS_ORIGINS` env var (comma-separated). Defaults to `localhost:8501,localhost:3000,localhost:8000`. Streamlit and the API coexist and can run simultaneously.
...
| `mypy --strict` scoped to `agents/` via `mypy-agents-strict.ini` | Strict type checking is activated only for the agents package (the orchestration core). `follow_imports = silent` for all non-agents packages prevents strict-check leakage into imported modules while preserving cross-module type information. Intermediate `mypy` (non-strict) continues across all 78 source files. Both steps run in CI and the pre-commit hook enforces the strict zone. |
