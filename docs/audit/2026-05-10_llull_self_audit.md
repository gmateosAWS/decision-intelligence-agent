...
| 17 | Typing and contracts rigor | 4 | **5** | **IMPROVED.** `mypy --strict` activated on `agents/` package (8 source files, 0 errors). `mypy-agents-strict.ini` isolates strict zone via `follow_imports = silent` for all non-agents packages. Fixes: full return-type annotations on all 4 node functions and helpers, `Optional[Any]` narrowing for `state.get("action")`, typed `tuple[str, Optional[str]]` for prompt builders, cross-zone `# type: ignore[no-untyped-call]` for `SystemModel`, `optimize_price`, `get_checkpointer`. Strict CI step added; pre-commit `mirrors-mypy` hook added (files: `^agents/.*\.py$`). Intermediate mypy step (78 files, 0 errors) continues unchanged. | `mypy-agents-strict.ini`; `agents/planner.py`; `agents/judge.py`; `agents/tools.py`; `agents/workflow.py`; `agents/runner.py`; `.github/workflows/ci.yml` (strict step); `.pre-commit-config.yaml` (mypy hook) | 🟢 |
