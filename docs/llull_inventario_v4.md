...
**Extendido 2026-05-17 — A/B testing de variantes de prompt** — `prompts/routing.py` (routing determinístico sha256-bucket, `@lru_cache` por stage, `invalidate_variant_cache()`); `PromptVariant` + `PromptVariantStatus` en `prompts/models.py`; `get_prompt_template()` → 3-tuple `(content, version, variant_label)`; CRUD de variantes en `prompts/registry.py` (`start_rollout`, `adjust_rollout`, `promote_to_champion`, `deprecate_variant`); `PromptVariantRow` ORM + 3 columnas `*_variant_label` en `agent_runs`; migraciones 008 + 009; 6 endpoints REST adicionales; tabla de variantes read-only en `ui/dashboard.py`; 330 tests pasan (27 nuevos). Deuda técnica: auto-promoción eval-gated diferida a 10.3.
