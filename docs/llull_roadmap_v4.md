...
| **5.9** GroundedTokens — guardrail anti-alucinación de entidades | Defense-in-depth para conversaciones largas. Importante en piloto interno porque el consultor probará con queries variadas: cuanto antes se detecten alucinaciones de entidades, antes se corrigen. Vocabularios alimentados desde el spec por dominio (no hardcoded). |
| **5.10** Memoria analítica activa tipada      | Pieza central. Sustituye "el LLM se acuerda de la conversación" por "el sistema mantiene estado tipado entre turnos". Habilita follow-ups precisos como "y si subo el marketing un 10%" sin ambigüedad. |
| **5.11** MemoryService — interfaz única        | Va con 5.10. Establece la disciplina arquitectónica desde día 1 (lint en CI bloquea acceso raw a `conversation_history` fuera de `memory/`). Barato hacerlo ahora, caro después. |
| **8.7.a** Cuotas y presupuestos por tenant     | Era el 8.7 original de v3. Se anticipa de I3 a I2A porque el piloto interno con datos reales necesita observabilidad de coste desde el primer momento (aunque sea con un solo "tenant" interno). |
| **8.7.b** Hard request-level budget ceilings   | **El item de coste más crítico.** Sin esto, una query patológica puede consumir 10+ llamadas LLM con stacked retries. Es el gap explícito de la auditoría AI Layer. Tope por defecto: 20 LLM calls, 60s wallclock, $1 USD por run. Configurable. |
