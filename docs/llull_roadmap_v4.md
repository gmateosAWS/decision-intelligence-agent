...
| ~~**5.13** User-driven state corrections~~ ✅ | ~~Ciclo explícito `propose_state_update` → revisión del usuario → `commit_state_update`.~~ Completado 2026-05-18: `agents/tools_registry.py` (cheap/expensive cost classes); `memory/proactive_confirmation.py` (signals `first_turn` + `thin_context`, `STATE_CONFIRMATION_SIGNALS` env var); `core/protocols/memory.py` real dataclasses; `memory/service.py` full propose/commit; `MemoryCoordinator` freeze/unfreeze; migration 010; `proactive_confirmation_gate` 6-node graph; bypass_gate flow; 3 new API endpoints; UI confirmation panel; 51 new tests (382 total). Hotfixes 2026-05-19 (PR #27): AND semantics en `memory/proactive_confirmation.py` (`triggered == active` — todas las señales activas deben disparar simultáneamente, no OR); reset explícito de checkpoint LangGraph en `agents/runner.py` (gate-only checkpoint bleed); `bypass_gate` añadido a `QueryRequest` y forwarded en `api/routers/query.py`; session state cleanup en `ui/session.py` (`handle_new_session` + `resume_session` limpian `_pending_proposal` / `_show_reactive_correction` / `_gate_bypass_prompt` / `_pending_query`). 417 tests. |
| **5.13.c** Reactive correction inline form (UI) | Formulario Streamlit que permite al usuario corregir slots del `ActiveAnalyticalState` directamente desde la UI sin necesidad de API. El backend está completo (5.13): `POST /v1/sessions/{id}/state/proposals` devuelve slots editables como propuesta identidad; `POST /v1/sessions/{id}/state/commits` aplica mutaciones aprobadas. Falta `render_reactive_correction(session_id, graph)` en `ui/components.py` y su integración en `ui/app.py`. Deuda técnica registrada en `docs/tech_debt.md`. |
