...
| ~~**5.13** User-driven state corrections~~ ✅ | ~~Ciclo explícito `propose_state_update` → revisión del usuario → `commit_state_update`.~~ Completado 2026-05-18: `agents/tools_registry.py` (cheap/expensive cost classes); `memory/proactive_confirmation.py` (signals `first_turn` + `thin_context`, `STATE_CONFIRMATION_SIGNALS` env var); `core/protocols/memory.py` real dataclasses; `memory/service.py` full propose/commit; `MemoryCoordinator` freeze/unfreeze; migration 010; `proactive_confirmation_gate` 6-node graph; bypass_gate flow; 3 new API endpoints; UI confirmation panel; 51 new tests (382 total). |
